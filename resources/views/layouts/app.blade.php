...
<div class="max-w-[70rem] flex flex-col mx-auto w-full h-full">
    ...
    <footer class="mt-auto text-center py-5">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @auth
                <div class="flex text-center justify-center gap-6 mb-1">
                    <x-pui.link :href="route('home')" icon="bi-house">{{ __('Home') }}</x-pui.link>
                    <x-pui.link :href="route('dashboard')" icon="bi-speedometer2">{{ __('Dashboard') }}</x-pui.link>
                </div>
            @endauth
            <p class="text-sm text-gray-400">
                Built with <i class="bi bi-heart text-xs"></i> by <span
